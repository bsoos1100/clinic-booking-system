...
# 1️⃣ تحديد مسار مطلق وثابت لملف قاعدة البيانات ليتجنب مشاكل الصلاحيات على السحاب
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DB_PATH = os.path.join(BASE_DIR, "clinic.db")

# 2️⃣ سحب بيانات الربط الخاصة بـ UltraMsg
INSTANCE_ID = "instance179370"
ULTRAMSG_TOKEN = "oda8qahugqvoo3zi"

# 3️⃣ إعداد قاعدة البيانات بالمسار المطلق الجديد
def init_db():
...
# إجبار النظام على بناء الجدول فوراً قبل أي خطوة أخرى
init_db()

# 4️⃣ دالة معالجة الرقم الليبي وإرسال رسالة التأكيد عبر UltraMsg
...
# 5️⃣ تهيئة الإعدادات العامة مع إجبار الواجهة على التلاؤم التلقائي مع الهواتف
...
st.sidebar.markdown("### 🏥 إدارة مركز النخبة")
...
if view_mode == "شاشة حجز المرضى":
    ...
    # استعلام الأوقات بأمان بعد التأكد من وجود الجدول تماماً
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    ...
    # توزيع الأوقات في عمودين متوازيين ليناسب شاشات الهواتف الطولية
    ...
    # 🗺️ شريط الموقع الجغرافي لـ طرابلس (حي الأندلس)
    st.markdown(f"""
...
# ---------------- الواجهة الثانية: لوحة تحكم موظف الاستقبال ----------------
    ...
    st.title("🖥️ شاشة استقبال micro-clinic")

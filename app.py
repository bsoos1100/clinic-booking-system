...
st.markdown("""
    ...
    <style>
        ...
        /* تصميم الأزرار التفاعلية للمواعيد */
        div.stButton > button {
            border-radius: 12px;
            font-size: 16px;
            font-weight: bold;
...
try:
    ...
    st.error("⚠️ خطأ في تهيئة المفاتيح السرية الحالية. يرجى لصق بيانات الحساب الجديد في الـ Secrets أولاً.")
    st.stop()
    ...
    try:
        ...
        for event in events:
            if "متاح" in event.get('summary', ''):
                ...
                # تفتيت فترة تواجد الطبيب لشرائح زمنية مدتها 20 دقيقة
...
def confirm_booking_to_sheet(day, slot, name, phone, doctor_name, patient_status):
    ...
    try:
        ...
        # حماية مزدوجة لمنع حجز نفس الـ 20 دقيقة لمرضين بالتزامن
...
# خطوة 1: اختيار الطبيب (تجنب الحقول الفارغة لعدم التسبب بأخطاء)
...
if doctor:
    ...
    if not all_days:
        st.warning("⚠️ لا توجد فترات عمل معلنة كـ 'متاح' لهذا الطبيب حالياً.")
    else:
        ...
        # تهيئة للاتصال بالشيت لقراءة الحالات وتلوين الأزرار
        sheets_client = gspread.authorize(creds)
        sheet = sheets_client.open(SPREADSHEET_NAME).sheet1
                    ...
                    if not p_name or not p_phone:
                        ...
                        with st.spinner("جاري حفظ البيانات وتأمين دورك..."):
                            ...
                            if success:
                                ...
                                """, unsafe_allow_html=True)
                                # تنظيف السيرفر للاستعداد للحجز التالي
